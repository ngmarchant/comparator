...
setClass("Levenshtein", contains = c("StringMeasure", "CppMeasure"), 
         ...
         validity = function(object) {
           ...
           if (!object@symmetric & equal_weights)
             errs <- c(errs, "`symmetric` must be TRUE for when equal weights are used")
           if (object@symmetric & !equal_weights)
             errs <- c(errs, "`symmetric` must be FALSE for when unequal weights are used")
           if (!object@tri_inequal & equal_weights & object@distance)
             errs <- c(errs, "`tri_inequal` must be TRUE for when equal weights are used and `distance` is TRUE")
           if (object@tri_inequal & !equal_weights)
             errs <- c(errs, "`tri_inequal` must be FALSE for when unequal weights are used")
